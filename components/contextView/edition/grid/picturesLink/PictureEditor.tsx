...
interface MediaEditorProps<T> {
  imageGroup: MediaObject;
  onChange: (fieldName: string, newValue: any) => void;
  removeElement: (imageGroup: T) => void;
}
    ...
    <div className="mx-auto max-w-2xl space-y-6 p-6">
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <div
          ...
          onClick={() => props.removeElement(props.imageGroup as T)}
        >
          <svg
        ...
        <div className="space-y-6">
          <FieldRenderer
            ...
            fieldName={`image_medias.${(props.imageGroup as MediaObject).number_position_image}.text_titre`}
            model={props.imageGroup as Record<string, any>}
            setField={props.onChange}
          />
          <FieldRenderer
            label="Lien de la carte"
            fieldName={`image_medias.${(props.imageGroup as MediaObject).number_position_image}.text_image_lien`}
            model={props.imageGroup as Record<string, any>}
            setField={props.onChange}
          />
          <FieldRenderer
            label="Image associée à la carte"
            fieldName={`image_medias.${(props.imageGroup as MediaObject).number_position_image}.image_image_url`}
            model={props.imageGroup as Record<string, any>}
            setField={props.onChange}
          />
      ...
      {debug && (
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          <pre className="text-xs overflow-auto">
            {JSON.stringify(props.imageGroup, null, 2)}
          </pre>
        </div>
      )}
    </div>
